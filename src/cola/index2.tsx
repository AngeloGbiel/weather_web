...
export default function App() {
  ...
  const Weather = async () => {
    ...
    const Photo = await fetch(`https://api.unsplash.com/search/photos?client_id=${KeyPhoto}&query=${city}`)
    const resPhoto = await Photo.json().
      then((data) => {
        console.log(data)
        const cityImgs = []
        for(let x = 0;x<4;x++){
          cityImgs.push(data.results[x].links.download)
        }
        setPhotoCity(cityImgs)
      })
    console.log(resWeather)

    //mostrar: nome da cidade, temperatura, feels_like, coordenadas, himidade%, pressão, Clima, vento
  }
  return (
    <Styled>
